...
const Reels = ({ navigation, route }) => {
  ...
  const swipeCountPrevious = useSharedValue(-1);
  const loadedVideosTill = useSharedValue(0);
  const unloadedVideosFrom = useSharedValue(0);
  ...
  const unlockGestureHandler = useAnimatedGestureHandler({
    ...
    onEnd: (e) => {
      ...
      if ((swipeCount.value < dataLength.value && swipeCount.value > 0)
        ...
        swipeCountPrevious.value = swipeCount.value;
        ...
        console.log(
          "loadedVideosTill.value", loadedVideosTill.value,
          "unloadedVideosFrom.value", unloadedVideosFrom.value,
        );

        console.log("Current video ===>", swipeCount.value);
        console.log("Previous video ===>", swipeCountPrevious.value);
        console.log("Total videos loaded==>", videosLoadedRef.current);
